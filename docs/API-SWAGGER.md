...
- Parametry:
  ...
  - `additionalData` (required) - JSON z dodatkowymi danymi (np. `nrKSeF`, `qrCode`, `companyLogoBase64` dla logotypu oraz opcjonalnie `qrCode2` dla trybu offline). Pole `nrKSeF` jest opcjonalne.
- Zwraca plik PDF
...
5. Wpisz dodatkowe dane:
   ```json
   {
    "nrKSeF": "123456789012345678",
     ...
     "companyLogoBase64": "data:image/png;base64,BASE64_LOGO_DATA"
   }
   ```
6. Kliknij **"Execute"**
